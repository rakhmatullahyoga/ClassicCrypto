    ...
    static String encryptVigenereExtended(String plain, String key) {
        ...
        for(int i=0; i<plain.length(); i++) {
            if(plain.charAt(i)==' ') {
                ...
                cipher[i] = getAscii(((int) plain.charAt(i) + (int) key.charAt((i-skip)%key.length())) % 256);
        }
        return String.valueOf(cipher);
    }
    ...
    static String decryptVigenereExtended(String cipher, String key) {
        ...
        for(int i=0; i<cipher.length(); i++) {
            if(cipher.charAt(i)==' ') {
                ...
                System.out.println((int) cipher.charAt(i));
                plain[i] = getAscii(((int) cipher.charAt(i) - (int) key.charAt((i-skip)%key.length()) + 256) % 256);
            }
        }
        return String.valueOf(plain);
    }
